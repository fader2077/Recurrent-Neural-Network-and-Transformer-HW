...
2 & 03-23 & 1,810 & -30 & 1,838 & +27.52 & 1.52\% & \checkmark & 30 \\
...
10& 04-02 & 1,810 & -45 & 1,861 & +50.83 & 2.81\% & $\times$ & 45 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
RMSE (TWD) & \textbf{40.27} & 41.63 & $\downarrow$ 3.3\% \\
MAE (TWD)  & 29.54          & 32.22 & $\downarrow$ 8.3\% \\
MAPE       & \textbf{1.62\%} & 1.77\% & $\downarrow$ 8.5\% \\
DirAcc     & 6/10 (60\%)    & —     & — \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{phase2_rolling_forecast.png}
\caption{Phase 2 Rolling Prediction vs Actual（2026-03-20 \textasciitilde{} 2026-04-02）。藍線為實際收盤價，紅線為滾動預測，灰線為 Naive Baseline（昨收即今預測）。每日標記顯示預測方向是否正確（\checkmark / \texttimes），10 日內 6 日方向正確（60\%）。}
\label{fig:phase2_rolling}
\end{figure}
...
全部規則通過：(1) 無透支；(2) 無放空；(3) 最終日強制清倉；(4) 積極參與（5 筆交易）。
...
\begin{itemize}
  ...
  \item \textbf{Phase 2（滾動預測）：} Rolling RMSE 40.27 TWD，比 Naive Baseline 低 3.3\%（41.63 TWD）；方向準確率 60\%（6/10）。
